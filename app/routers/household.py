...
def update_presence(body: PresenceUpdate, session: Session = Depends(get_session)):
    ...
    if existing:
        ...
        existing = Presence(profile_id=body.profile_id, location=body.location, updated_at=datetime.utcnow())
        session.add(existing)
...
def update_permission(rule_id: int, body: PermissionRuleUpdate, session: Session = Depends(get_session)):
